...
const SearchPage = () => {
    ...
    // FIXED: Calculate total quantity with optional parameters
    const calculateTotalQuantity = (productId, packId = null, unitsQty = null) => {
        ...
        const selectedPack = packTypes.find(pack => pack._id === packIdToUse)
        const packQuantity = selectedPack ? parseInt(selectedPack.quantity) : 1

        return packQuantity * unitsToUse
    }

    // FIXED: Check stock level with optional parameters
    ...
    const fetchSearchResults = async (page = currentPage, itemsPerPage = perpageItems, sortOption = sortBy) => {
        try {
            ...
            const response = await axiosInstance.get('products/search-products', {
                params: queryParams
            })
                        ...
                        <div className="bg-white rounded-lg pb-3 lg:pb-4 mb-4 lg:mb-0 ">
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 lg:gap-4">
                                <h2 className="text-lg lg:text-[1.2rem] font-[400] text-black">
                                    Search Results <span className="text-[#000000]/60">({totalProducts})</span>
                                </h2>
