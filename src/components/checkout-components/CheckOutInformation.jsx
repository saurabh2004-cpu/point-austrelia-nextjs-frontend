...
import { ChevronDown, Circle, CloudHail, LockIcon, Plus } from 'lucide-react';
        ...
        try {
            ...
            if (response.data.statusCode === 200) {
                ...
                if (response.data.data.cards.length) {
                    setSelectedCardId(response.data.data.cards[0]._id)
                }
        ...
        if (cardData !== null) {
            cardData?.cards?.map((card) => {
                if (card.defaultCard) {
                    ...
                    setSelectedCardId(cardData.cards[0]._id)
                }
            })

            const card = cardData?.cards?.find((card) => card._id === selectedCardId);
